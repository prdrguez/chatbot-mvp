...
def test_kb_grounding_injects_context_and_sources():
    ...
    response = service.send_message(
        ...
        user_context={"kb_text": kb_text, "kb_name": "securin.txt"},
    )
...
def test_kb_grounding_returns_strict_message_without_evidence():
    ...
    assert response.startswith("No encuentro eso en la política cargada.")
    assert "Fuentes: Sin evidencia recuperada" in response
...
def test_kb_grounding_returns_strict_message_with_empty_kb():
    ...
    response = service.send_message(
        ...
        conversation_history=[],
        user_context={"kb_text": "", "kb_name": "securin.txt"},
    )
